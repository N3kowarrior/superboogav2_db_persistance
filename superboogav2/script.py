...
def _feed_file_into_collector(file):
    ...
    try:
        ...
        logger.warn(f'Failed to load contents of file, due to:{str(e)}')
    #Added to make the certein "error" less scary
    try:
        process_and_add_to_collector(text, collector, False, create_metadata_source('file'))
    except Exception as e:
        logger.warn(f'You might have clicked on button load data without selecting file, or inspect this error furter:\n{e}')
    yield '### Done.'
